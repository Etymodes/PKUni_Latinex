...
当前任务：微信原生客户端位于 Draft PR #21；同一网站邮箱账号登录及网页收藏到小程序同步已由用户确认。网页版旧缓存使已取消收藏复活的问题已在独立 PR #22 修复；网页回归、TypeScript、生产构建与 Cloudflare Preview 已通过。2026-09-17 用户确认“小程序取消收藏 → 预览网页刷新”后收藏成功消失，该项跨端删除验收通过。PR #22 待用户单独授权合并与正式发布；当前正式站仍为 PR #20，小程序未上传、未提审、未发布，真机及进度、词汇统计同步仍待验收。
