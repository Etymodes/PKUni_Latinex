...
当前任务：微信个人主体原生工程已在用户 Windows 构建成功，并进入微信模拟器。用户截图指出等级方卡过大且纵向堆叠；本轮将其替换为小程序独立 `level-selector` 组件，四等宽横排按钮固定 104rpx 高，中文“初／凡／高／母”、英文“C／F／G／M”，下方显示所选等级完整名称。保留四色与独立字母 PNG，选择继续走原偏好保存路径。更新位于 Draft PR #21 的 `agent/pikku-wechat-miniprogram`，待用户拉取后复核原生显示；未上传、未提审、未发布小程序，网站保持 PR #20 生产版本。
...
- 首个原生工程位于 `wechat/`，构建命令 `npm run build:wechat`；需要在微信后台实际验证网站 API 与现有身份服务的 request 合法域名可保存。当前尚未验证域名准入及真机访问，不允许把关闭域名检查记为发布通过。
