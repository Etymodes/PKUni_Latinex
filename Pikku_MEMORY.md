...
当前状态：正在把 Sites v7 已有的中英显示语言、七学习语言和 C/F/G/M 合流到 Cloudflare/GitHub；工作分支 `agent/pikku-restore-cfgm-i18n`，已建 Draft PR #19，待 Cloudflare Preview 验收及正式发布。9 月 16 日上线的 PR #18 只完成剧情原型与选项洗牌，并非包含所有既有功能的最新版。
...
- 原 GitHub／Cloudflare 第一版为拉丁语、日语、西班牙语。Sites v7 已有普通话、美式英语、拉丁语、日语、西班牙语、古希腊语、俄语七个入口，本轮恢复到同一账号和网站外壳，尚待 Cloudflare 发布。
- 显示语言 `zh-CN`／`en` 自主切换与学习语言分开；已有 Sites 实现正在合流，不能继续记为“尚未实现”。
...
- 本轮七语言公开等级统一为 `C/F/G/M`，默认 `C`；恢复的是 Sites v7 已有产品分级，Cloudflare 版本仍待发布。
...
- GitHub 远端 `main` 当前为 `05698584d7dcbbf78555a0c3fdf7f3ead706822d`（PR #18 发布回执文档）；上次已验收生产应用基线为 `b9545df6126ffd501526e5b3f3ac281612971332`。该应用基线只包含 GitHub 线已有功能、剧情原型和选项洗牌，不包含当时未同步的 Sites 功能。
- 遗漏来源已核实：Sites `pkuni-latinex` v7，2026-08-20 已发布，源码提交 `b215d9bbefed0a8df42226d69ca9660397bcacb2`。该独立源码含中英显示语言、七语言入口与 CFGM；此前仅查 GitHub 就断言这些功能未实现是错误的。当前在 `agent/pikku-restore-cfgm-i18n` 合流，未将恢复代码记为已在正式域名上线。
...
- 当前优先项：把 Sites v7 中英显示语言、七语言与 CFGM 合流至既有 Cloudflare 账号／API 架构，完成 Preview、兼容与正式站验收后发布；PR #18 上线是已完成的历史步骤，不代表本轮最新版已发布。
- P4：多语言管理员题库管理。
- P5：资源、词典、知识图谱。
- P6：带审核能力的社区。
- 中英底座、语言下拉框、七语言种子与 CFGM 本轮恢复；完整古希腊语课程、跨语言联学、探索地图及 M1 账号级剧情事件仍待后续排期，不混记为已实现。
...
### 2026-09-17（UTC）Sites v7 恢复记录（待 Cloudflare 验收／发布）
...
- 已有证据：数据／旧词卡／审核回归 14 项通过，独立源码比对确认原卡和恢复种子内容保留；主代理确认整合检查 43/43 与 Cloudflare 生产构建通过，语言切换索引越界及词汇测量重复词阻塞已修复。界面文案补齐后的构建仍在重跑，Preview／正式发布及本轮 Firefox 尚无通过回执。
- 当前下一步：完成整合检查和 Cloudflare Preview 的语言切换、刷新、题池／词卡、旧等级账号偏好与洗牌判分验收，再按本轮用户授权发布正式域名并回填真实结果。当前没有本轮正式部署回执；Augusta Firefox 随机化专项仍未补测。
...
- 2026-09-17（UTC）：用户指出显示语言和 CFGM 遗漏后，确认 Sites v7 独立源码已有这些功能；纠正“PR #18 即最新版／功能未实现”的判断，按恢复源合流七语言与 CFGM，保留 Cloudflare 账号数据和旧等级兼容。当前为待 Preview 验收／正式发布，不把恢复中的代码、未完成课程或设计库全部内容记成已上线。
