...
当前任务：微信原生客户端位于 Draft PR #21；用户已确认同一网站邮箱账号登录成功、网页收藏可同步到小程序，但小程序取消收藏后网页未同步删除。已复现网页版 `refreshAccount` 将浏览器旧收藏与云端列表无条件取并集、再写回云端，导致已删除收藏复活。网页修复单独放在 `agent/pikku-bookmark-sync`（基于 `main@60c2068`），网页回归、TypeScript 与生产构建已通过，待 Preview 和用户跨端复测；当前正式站仍为 PR #20，小程序未上传、未提审、未发布。
