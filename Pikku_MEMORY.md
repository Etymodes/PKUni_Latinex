...
当前状态：PR #22 已按用户本轮明确授权合并为 `3e2ac5b7bd66618e786240306fecaebf9e5011f0`，Cloudflare Workers 生产构建于 `2026-09-17T12:08:06Z` 成功。正式 `https://pikku.qzz.io/` 的游客收藏添加、取消、刷新后空列表通过云端 Chrome 验收，未见应用错误（仅浏览器扩展日志）。真实账号跨端删除已在发布前 Preview 由用户确认，未另行宣称正式站真实账号、Firefox 或真机验收通过。 PR #20 四色 CFGM 卡片及 PR #19 中英显示语言、七语言入口继续保留。

当前任务：继续完成 Draft PR #21 微信原生客户端，开发分支已吸收正式站 `main@50b5490`（含已发布 PR #22 收藏修复），保留同一账号与数据接口。补齐手机语言切换布局、首页开始练习的空筛选恢复及原生客户端到后端的同步回归；下一步由用户在专用 Windows worktree 更新构建，继续真机和真实账号的进度、词汇统计验收。小程序仍未上传、未提审、未发布。

本轮发布收口：固定 Drive 周报告目录复查仍只有现有汇总文件（修改时间 `2026-09-17T04:38:41.182Z`），没有新增补丁；PR #22 不含原生小程序、数据库或 Worker 接口变更。用户本轮发布授权仅适用于 PR #22，不改变后续 PR 的通用发布约定。

本轮原生收口：已吸收 `main@50b5490` 并保留小程序原有功能。顶部语言按钮改为固定触控尺寸，“我的 → 显示语言”整行排版；首页开始练习会清除上一轮空收藏／搜索／题型筛选，而练习页仍尊重当前筛选。新增原生 Page → 实际 API 客户端 → Worker → 内存 SQLite 同步回归，覆盖选择题、翻译自评、记得／忘了、连续点击只写一次、网页同一统计接口读回及退出／换号隔离。`npm run test:wechat` 33/33、TypeScript 和格式检查通过；构建保留 459 道题、78 张词卡、7 种学习语言，包体 543110 字节。以上为本地隔离验证，未代替微信更新后布局、真机或真实账号进度／背词验收。
...
- 原 GitHub／Cloudflare 第一版为拉丁语、日语、西班牙语。Sites v7 已有普通话、美式英语、拉丁语、日语、西班牙语、古希腊语、俄语七个入口，本轮恢复到同一账号和网站外壳，已通过 PR #19 发布至 Cloudflare 正式域名。
- 显示语言 `zh-CN`／`en` 自主切换与学习语言分开；已有 Sites 实现已通过 PR #19 合流并发布，不能继续记为“尚未实现”。
- 学习语言下拉框及 `grc` 入口已有实现，搜索功能仍属后续工作；古希腊语当前只有 C/F/G/M 各一道种子题，不表示完整课程已建成。
...
- 微信小程序使用单独 Windows worktree `Documents\Pikku_WeChat`，基于 `main@60c2068` 创建同名功能分支；原 `Documents\PKUni_Latinex` 工作目录保留。用户电脑已确认 Node、Git 和微信开发者工具存在。
- 首个原生工程位于 `wechat/`，构建命令 `npm run build:wechat`；2026-09-17 用户截图已确认网站 API 与现有身份服务的 request 合法域名保存成功，微信开发工具刷新配置后域名错误消失。真机访问、账号登录与双向同步仍待验证，不允许把关闭域名检查记为发布通过。
...
- 学习进度采取本地优先、登录后云端同步。
- 微信原生首版的游客记录保存在本机，登录后通过现有邮箱账号的 UUID 读写网站学习状态、收藏、词汇统计和偏好。客户端生成题库保持原题号与词汇统计键；登录写入获得接口确认后更新统计，失败不自动重放词汇增量。显示语言独立存于本机；不会把游客记录自动并入账号。
- 小程序首版覆盖原生练习、错题、收藏、背词和资源书目；剧情、模拟考试、管理与社区页面尚未移植。网站微信扫码 OAuth 与小程序 `wx.login` 是不同协议，本轮采用已存在的邮箱密码登录，不创建第二套身份或数据库。
...
- 本轮背词公开等级采用 C→F→G→M 累积覆盖，并纳入路由对应的旧等级词卡。旧拉丁语混合、JLPT 与 CEFR 选择器语义仍保留在兼容函数中，不改写旧卡记录。
- 多语言采用共享网站外壳＋语言配置＋各语言独立题库/资源/等级。
- 本轮核心学习语言类型：`"zh-mandarin" | "en-us" | "la" | "ja" | "es" | "grc" | "ru"`。
- `uiLocale` 与学习语言已在恢复代码中分离；显示语言独立本机持久化，学习语言／等级仍沿用账号偏好。`courseTrack` 与完整内容翻译仍为后续工作，不能把显示语言切换等同于所有既有题干、解析均有双语版本。
...
- 本轮七语言公开等级统一为 `C/F/G/M`，默认 `C`；恢复的是 Sites v7 已有产品分级，Cloudflare 正式域名已出现四档入口。
- 旧数据内部路由：拉丁语 `elementary→C`、`intermediate→F`、`advanced→G`、历史 `mixed→C`；日语 `n4→C`、`n3→F`、`n2→G`、`n1→M`；西语 `a1→C`、`a2/b1→F`、`b2/c1→G`、`c2→M`。这些仅是兼容路由，不声明考试等级等价。
- Sites 种子文件中的旧 `mixed` 指 M 档，恢复时明确转为 `M`；不得把原 Cloudflare 的混合组卷偏好擅自解释为 M。旧题号、等级原值、审核状态、账号记录和词卡统计键保留。
- 北大考纲、JLPT、CEFR 继续作为外部内容／资源标签。古希腊语完整五域课程仍待建设，不因四道种子题就视为完成。
...
- 切换学习语言时保留当前兼容模块，并在当前页面会话内记住各语言最近等级；本轮统一为 CFGM 后首次切换保留当前档。题库、测量和统计按所学语言过滤，非拉丁语剧情显示本语种 outline，真题档案切到其他语言仍回落资源页。
...
- GitHub `main` 的已验收生产应用基线为 `cca9181a61c678fa5fdf543694798b2317be4c6d`（PR #20 squash merge）；Workers Builds 与正式站 Chrome 验收通过。此前 PR #19 基线 `fdd8ccf1419f4a61551737f78465afff674be50b` 和 PR #18 基线 `b9545df6126ffd501526e5b3f3ac281612971332` 的历史回执保留。
- 遗漏来源已核实：Sites `pkuni-latinex` v7，2026-08-20 已发布，源码提交 `b215d9bbefed0a8df42226d69ca9660397bcacb2`。该独立源码含中英显示语言、七语言入口与 CFGM；此前仅查 GitHub 就断言这些功能未实现是错误的。恢复分支 `agent/pikku-restore-cfgm-i18n` 已通过 PR #19 合并并发布至正式域名。
...
- P3.3 来源分支：`agent/pikku-p3-3-story-prototype`，从 `main@f177f1d` 创建，已通过 PR #18 合并；只承载第一条自然习得剧情静态原型、测试和对应文档，不新增后端、数据库表或游戏引擎。
- PR #18：`feat: add first Pikku story lesson` 已于 2026-09-16（UTC）按用户本轮明确授权合入 `main@b9545df`，合并前 head 为 `208d7bef`。核心功能提交为 `88fa846`，选项随机化提交为 `af180dd`；27/27 Node 测试、Cloudflare Preview 与云端 Chrome 定向验收已通过。原定 Augusta 8/8 与 Firefox 剧情项目已有通过记录，但随机化后的 Augusta Firefox 专项没有补测，不记作通过。本轮授权覆盖该 PR 的合并及 `pikku.qzz.io` 发布，不改变后续 PR 的通用验收与授权规则。
- PR #19 已按用户明确授权合并，merge SHA 为 `fdd8ccf1419f4a61551737f78465afff674be50b`，最终 head 为 `6a8559566a8fac5061a181d9302c23c851df3278`；该 head 的 Cloudflare CI 于 `2026-09-17 03:33:44 UTC` 成功。本轮用户明确以云端 Chrome 验收为准；Firefox 未补测、真实账号跨设备未验收，不修改下列通用规则。
- PR #20 按用户新增的明确“发布”授权于 `2026-09-17T04:17:22Z` squash merge 为 `cca9181a61c678fa5fdf543694798b2317be4c6d`；Cloudflare Workers Builds 于 `2026-09-17T04:18:33Z` 成功，build／deploy 成功，正式站 Chrome 验收通过；不改通用发布规则，不冒充 Firefox 或真实账号跨设备验收。
- 不直接在 `main` 开发；功能分支必须通过 Preview、Augusta 批量检查和 Firefox 复验后才可合并。

## 7. 当前阶段与最小路线

以下 P0–P3.3 为原 GitHub 线历史验收；本轮 Sites 恢复的待验收范围另列，不能沿用旧测试结果。P0 基线整理已完成。P1 多语言外壳当时通过 Augusta/Firefox 验收：
...
- P3.3：首个拉丁语原型“香山碑文与版本线索”及全站客观题展示顺序随机化已通过 PR #18 合入 `main`，正式域名发布及线上定向回归完成。共享 Fisher–Yates 函数稳定原答案身份和判分逻辑，不改题库、账号数据或数据库。27/27 Node 测试、TypeScript、Next/Sites、GitHub Pages、Cloudflare、Wrangler dry-run及随机化 Preview 的云端 Chrome 定向复验已有通过记录；Augusta Firefox 随机化专项仍未补测，本次按用户明确授权发布。原型采用中性第二人称与六阶段闭环，仍为 M0，只在本次章节内计分，不写入账号统计。
- 本轮恢复发布已完成：PR #19 合流、授权合并、生产部署与云端 Chrome 正式站定向回归均已完成，结果见本轮恢复记录；后续仍按下列内容边界和排期推进。
- P4：多语言管理员题库管理。
- P5：资源、词典、知识图谱。
- P6：带审核能力的社区。
- 中英底座、语言下拉框、七语言种子与 CFGM 已通过 PR #19 恢复合并并发布；完整古希腊语课程、跨语言联学、探索地图及 M1 账号级剧情事件仍待后续排期，不混记为已实现。
...
- 公开分级为 CFGM；旧日语 N4–N1、西语 CEFR A1–C2 保留为内容来源和兼容标签。
...
2026-09-16（UTC）发布前补丁复查：完整只读审阅以下四份新增任务书；它们仍引用旧 `main@f177f1d`／PR #18 head `208d7bef`，不是比拟发布代码更新的应用版本。

| 补丁文件 | Drive 修改时间（UTC） | 处理结论 |
| --- | --- | --- |
| `Pikku_WeeklyPatch_2026-08-16.md` | `2026-08-16T14:08:35.013Z` | 审计与未实施任务进入后续排期；不作为本次上线内容。 |
| `Pikku_WeeklyPatch_2026-08-30.md` | `2026-08-30T14:08:15.982Z` | B1／B2 材料保留为高级候选，不覆盖用户 8 月 28 日确定的西语从零、以高频词为中心的学习策略。 |
| `Pikku_WeeklyPatch_2026-09-06.md` | `2026-09-06T14:06:54.723Z` | CORPES rank 1–16 候选批次保留；频次表简版字段契约由 9 月 13 日版补全。 |
| `Pikku_WeeklyPatch_2026-09-13.md` | `2026-09-13T14:08:45.635Z` | CORPES rank 17–25 作为另一批保留；字段契约增加 `skipped`／`skipReason` 并保留原始 rank。 |

- 接受并排期：管理员校验、证据字段、候选题词和 CORPES 频次表；`evidenceStatus` 枚举在实现前统一，不混用各任务书的不同版本。
- 被新版取代：四份任务书中的旧 main／开放 PR 状态，以本轮 PR #18 实际合并记录为准。9 月 13 日字段补全取代 9 月 6 日的简版契约，两个不同 rank 批次均保留。
- 9 月 16 日 PR #18 发布范围：上述四份补丁均未追加实现；该次 Cloudflare 应用仅有拉丁语、日语、西班牙语。此为当次部署范围，不能推断独立 Sites 七语言尚未实现；9 月 17 日恢复情况见本轮记录，设计库 v4.1 与 18 人设定仍不能整体宣称上线。
...
- Sites 与 GitHub 可能存在未同步的独立功能线；恢复时必须同时核对 GitHub main／分支／PR、Sites 发布版本与源码，再保留双方功能合流。不能未经比较就以 GitHub 覆盖 Sites，或仅凭 GitHub 未查到便断言功能未实现。`.openai/hosting.json` 若只有工具键顺序变化，不纳入功能提交。
...
### 2026-09-16（UTC）生产发布记录

- 用户本轮明确授权合并 PR #18 并发布 `pikku.qzz.io`；GitHub 合并成功，merge SHA 为 `b9545df6126ffd501526e5b3f3ac281612971332`，来源 head 为 `208d7bef25f8142548373df6fdd5fbca5b2a0486`。
- 来源 head 相对已记录通过云端 Chrome 复验的 `bba672b` 仅增加 memo／任务书变更，应用代码一致；27/27 Node 测试、Cloudflare Preview 与云端 Chrome 已验收。随机化后的 Augusta Firefox 专项没有补测，不写成通过。
- 四份新补丁已完整审阅并登记后续队列，本次发布仅包含 PR #18 已实现代码；不新增语言、账号级剧情存档、管理后台扩展或未经审校的素材内容。
- 生产结果：Cloudflare Workers Builds 对 merge `b9545df` 构建与部署成功，完成时间 `2026-09-16T19:22:46Z`；GitHub build／deploy 状态成功。正式站 `https://pikku.qzz.io/` 已出现“进入剧情”。
- 正式站定向回归：剧情 Hook → Explore（2/6）选择正确项 C 后显示“线索成立”，前进／返回保持选项顺序，重开后正确项由 C 变为 B；西语 A1 正确项 `soy` 两次进入由第 4 位变为第 3 位，点击及数字 3＋Enter 均显示 `Recte`。排除浏览器扩展日志后页面错误为空，未见应用或 hydration 错误。
- 验证范围：本次未测试真实登录；直接导航 API 时工具报 `ERR_BLOCKED_BY_CLIENT`，因此未将 API 记为验收通过。本次 Worker 代码未变；Augusta Firefox 随机化专项仍保留未补测状态。

### 2026-09-17（UTC）Sites v7 恢复记录（PR #19 已合并发布）

- 纠错：9 月 16 日把 PR #18 当成“最新版”发布，遗漏了已独立发布的 Sites v7。其生产回执仍有效，但不能证明中英显示语言、七语言或 CFGM 已在 `pikku.qzz.io` 上线。
- 本轮从 `b215d9bbefed0a8df42226d69ca9660397bcacb2` 恢复中英显示语言（`zh-CN`／`en`）、七语言下拉入口及 CFGM，搜索仍待后续；保留 GitHub 线的 Supabase／Worker／D1、账号偏好、题库审核、资源隔离和洗牌判分。显示语言与所学语言独立，选择器排除当前显示语言对应的学习语言。
- 内容边界：保留 25 道 Sites 种子题的稳定 ID 和内容；普通话、美式英语、古希腊语、俄语各级各 1 道，共各 4 道。原 56 张 adaptive 词卡逐字段保留，新增去重后共 78 张；恢复词卡尚无例句时仍以词条自身作为语境，不声称已扩充完整课程。
- 拉丁语继续使用已有六阶段剧情原型；非拉丁语仅为本语言剧情 outline，未实现完整七语剧情、18 人角色互动或 M1 账号剧情存档。显示语言切换也不表示既有题目解析已全部翻译。
- 兼容边界：旧题号、审核状态、账号与词汇统计键不变；Worker 接受七语 CFGM，并保留原三语历史等级读写，未批量改写旧数据库记录。全部兼容映射只用于内部路由。
- 已有证据：数据／旧词卡／审核回归 14 项通过，独立源码比对确认原卡和恢复种子内容保留；整合检查 43/43 与 Cloudflare 生产构建通过，语言切换索引越界及词汇测量重复词阻塞已修复。云端 Chrome Preview 定向验收通过；最终 head `6a8559566a8fac5061a181d9302c23c851df3278` 的 Cloudflare CI 于 `2026-09-17 03:33:44 UTC` 成功。
- 合并与授权：用户明确同意合并 PR #19 并发布，本轮以云端 Chrome 验收为准；实际 merge SHA 为 `fdd8ccf1419f4a61551737f78465afff674be50b`，GitHub `merged_at` 为 `2026-09-17T03:41:09Z`。本次授权不改通用发布规则，也不代表 Firefox 或真实账号跨设备验收通过。
- 生产回执：merge `fdd8ccf` 的 Cloudflare Workers Builds 完成成功，时间为 `2026-09-17T03:42:35Z`。云端 Chrome 重新载入 `https://pikku.qzz.io/` 后，DOM 已明确出现简中／English 显示语言入口及 CFGM 四档；此回执来自 Workers 与正式域名，不以 GitHub Pages 构建成功代替。
- 正式站定向回归：中→English 后导航、按钮与 CFGM 名称切换；英文学习列表提供普通话及 la／ja／es／grc／ru，排除 en-us。选择俄语 M 后出现 `двусмысленность` 与四个答案，刷新后 English／ru／M 保留。切回中文时仍保留俄语 M，列表提供 en-us 及 la／ja／es／grc／ru，排除普通话，M 显示“准母语级”。验收结束恢复中文／拉丁语／C。
- 错误与验收边界：正式站 error 日志查询（上限 500 条）剔除浏览器扩展后应用错误为 0；本轮按用户指定以云端 Chrome 验收收口。Augusta Firefox 随机化专项仍未补测，真实账号跨设备未验收；这两项不写为通过。
...
- 2026-09-16（UTC）：本轮用户明确授权后，PR #18 合并为 `b9545df` 并成功发布至 `pikku.qzz.io`，线上剧情与普通题洗牌／判分定向回归通过。同步修正两份文档的当前基线和下一步，保留历史 Draft 记录及未补测的 Augusta Firefox 随机化专项，登记四份新增周补丁为已审阅、未实施；真实登录与 API 未在本轮验收通过，详见生产发布记录。
- 2026-09-17（UTC）：用户指出显示语言和 CFGM 遗漏后，确认 Sites v7 独立源码已有这些功能；纠正“PR #18 即最新版／功能未实现”的判断，按恢复源合流七语言与 CFGM，保留 Cloudflare 账号数据和旧等级兼容。当时为待 Preview 验收／正式发布，不把恢复中的代码、未完成课程或设计库全部内容记成已上线。
