...
- 古希腊语使用 `grc`，与现代希腊语 `el` 分开；建议候选首发轨道为阿提卡／古典希腊语，仍待用户确认。
...
- 古希腊语（规划中）：不套用 CEFR；候选为字母与多调符号、形态基础、阿提卡散文、原典进阶四阶段。
...
- P5：资源、词典、知识图谱。
- P6：带审核能力的社区。
- 下一代多语言规划闸门：P3／P3.1 收口后，根据用户意见再排中英双语底座、语言下拉框、古希腊语占位、跨语言联学和探索型界面；本轮研究不实现代码。
...
- 古希腊语入口使用 `grc`；阿提卡／古典、通用和荷马希腊语应作为不同课程轨道，不混成一个等级体系。
- 未来联学候选包括罗曼—拉丁、日耳曼、闪米特、突厥、南岛、班图语言群，以及汉语变体—日韩越汉字词。
- 各模式不能共用一套“相似词”玩法：分别处理规律音变、词根词型、黏着形态、名词类别、历史音层、借词层和假朋友。
- 产品研究报告：`docs/Pikku_Global_Language_Learning_Research_v1.md`。报告建议“学习台＋探索地图”、词源侦探和文献修复等候选交互，但具体优先级等待用户确认。
