...
当前任务：用户要求逐步用一行 PowerShell 命令发布微信小程序版 Pikku，并与正式网站同步数据。已确认小程序主体为个人；在独立 `agent/pikku-wechat-miniprogram` 分支准备原生工程，邮箱登录复用网站身份与接口。当前为待用户电脑及真机验收版本，未上传、未提审、未发布小程序；网站仍保持 PR #20 生产版本。
