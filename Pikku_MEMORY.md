...
- 2026-08-10 队列动态盘点：最初发现 22 个源文件，整理期间新增 1 个，累计 23 个；首批完成 5 个源文件后，根目录剩 18 个未处理源文件。以后每轮重新盘点，不能把这个数字当成永久常量。
...
- 首批共生成 6 个教学索引／元数据文件及 2 个操作文件；两份已检查 PDF 均无可独立提取的栅格图片；受限材料全文输出数为 0。所有 5 个原文件均在输出回读后移动，未删除任何源文件。
...
- 2026-08-10：启动素材库持续摄取。累计盘点 23 个源文件，首批按“登记／版权分级 → 识别 → 去重 → 建索引 → 回读 → 移动”完成 5 个，剩余 18 个；建立 `语言类别／知识点／资源类型` 输出树、README、TSV 清单和三个 `已处理` 分流目录。两组 `(1)` 文件经 SHA-256 确认为精确重复；三份受限／许可不明材料只生成元数据与知识点索引，没有公开转换全文。补丁目录无更新，PR #18 继续保持 Draft，随机化 Firefox 短复验延后到恢复发布工作时进行。
