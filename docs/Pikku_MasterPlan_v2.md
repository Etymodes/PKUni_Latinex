...
工程默认启用域名检查；用户已在小程序后台保存现有 API 和身份服务的 request 合法域名，刷新配置并重新编译后红色域名错误消失。此后网页版邮箱登录出现 `NetworkError`，管理接口确认共用身份项目为 `INACTIVE`，已恢复为 `ACTIVE_HEALTHY`，认证健康检查与登录接口 CORS 预检均返回 HTTP 200，暂停具体原因未核实。当前待原账号登录、网站与小程序双向读写、真机及后台提审条件验收。详见 `wechat/README.md`。
