...
## 微信原生首版（2026-09-17，待验收）

代码在 `wechat/`，通过 `npm run build:wechat` 从现有网站源码生成 459 道题、78 张词卡、七语配置及学习算法。保留原 ID、审核标记、等级兼容规则与词汇统计键；原生中英界面、CFGM 四色图标卡片、练习／翻译自评、错题、收藏、背词与资源书目已准备。

Windows 构建已成功。用户微信模拟器截图显示旧等级卡过大并纵向堆叠；小程序改用独立 `level-selector`：单排四等宽按钮、高度 104rpx，中文初／凡／高／母、英文 CFGM，下方保留完整所选等级名、四色和原字母 PNG。用户已更新至 `c20d6e6` 并提供模拟器截图，确认中文紧凑布局显示正常；等级仍调用原有本机／云端偏好保存，不改变网站设计。

学习状态、收藏、背词计数、学习语言／等级／模式调用现有 Worker API，身份使用网站同一个邮箱账号的 Supabase UUID。未新增数据库、服务端密钥或微信身份转换。游客记录与显示语言仅保存在本机，游客不自动并入账号；剧情／模拟考试／管理／社区界面尚未移植。

工程默认启用域名检查；用户已在小程序后台保存现有 API 和身份服务的 request 合法域名，刷新配置并重新编译后红色域名错误消失。此后网页版邮箱登录出现 `NetworkError`，管理接口确认共用身份项目为 `INACTIVE`，已恢复为 `ACTIVE_HEALTHY`，认证健康检查与登录接口 CORS 预检均返回 HTTP 200，暂停具体原因未核实。用户已确认原账号登录、网页收藏到小程序同步，以及小程序取消收藏到修复预览网页删除。PR #22 已正式发布；真实账号的进度、背词统计、真机及后台提审条件仍待验收。详见 `wechat/README.md`。

本轮固定 Drive 周报告目录检查：按用户此前授权合并并核验的 14 份报告已统一为 `多语言学习周报与Pikku周补丁汇总.md`，原件已删除；本轮复查目录仅该汇总，未发现新补丁，不重复导入其私人学习内容。
