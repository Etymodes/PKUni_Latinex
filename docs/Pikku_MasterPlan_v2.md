...
当前任务：网站等级卡片已发布；微信个人主体原生小程序在独立 worktree／分支复用网站邮箱身份与学习接口，按一条 PowerShell 命令一轮推进。Windows 构建、模拟器紧凑等级模块与后台 request 合法域名保存已获用户截图验证，刷新配置后的域名错误已消失。登录排障发现现有 Supabase 身份项目为 `INACTIVE`，已恢复为 `ACTIVE_HEALTHY`，认证健康检查与登录接口 CORS 预检均为 HTTP 200；原账号登录、双向同步和真机验收待用户完成。未上传、未提审、未发布小程序。
