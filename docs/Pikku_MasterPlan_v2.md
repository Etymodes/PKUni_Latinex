...
- 2026-08-10 动态盘点最初为 22 个源文件，期间新增《西塞罗曰》和 `Learn to Read Latin`，累计 24 个；完成 7 个后，根目录剩余 17 个未处理源文件。已建立拉丁语输出树、操作协议、TSV 清单和三个 `已处理` 分流目录；所有移动均已回读验证，未删除文件。
...
- 后续处理包括：拉丁—法语 LLPSI 词汇 PDF 只生成来源、词汇结构和术语索引；凯撒《高卢战记》卷 I、IV、V 学习选本词汇只生成来源及作品词汇索引，并按共和国时期—作者—作品—选本归档。二者均为 `personal-use-only`；凯撒公共领域原文与现代编辑／英语释义的权利必须分别判断。累计四份已检查 PDF 均无独立栅格图片，11 个教学输出均为元数据或原创结构索引，受限材料全文输出仍为零。
...
1. 当前集中执行 Google Drive 素材队列：每轮从剩余 17 个源文件中锁定一个，先做版权核验，再按固定目录拆分或只建索引；每轮更新 README／TSV 清单、回读输出并在完成后移动原文件。
