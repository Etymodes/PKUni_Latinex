...
当前任务：网站等级卡片已发布；新增微信个人主体原生小程序，独立 worktree／分支复用网站邮箱身份与学习接口，按一条 PowerShell 命令一轮推进。首个工程已准备，当前待微信开发者工具、合法域名、双向账号同步和真机验收；未上传、未提审、未发布小程序。
...
Windows 构建已成功。用户微信模拟器截图显示旧等级卡过大并纵向堆叠；小程序改用独立 `level-selector`：单排四等宽按钮、高度 104rpx，中文初／凡／高／母、英文 CFGM，下方保留完整所选等级名、四色和原字母 PNG。新布局待用户模拟器复核，等级仍调用原有本机／云端偏好保存，不改变网站设计。
...
工程默认启用域名检查；正式发布必须先确认小程序后台可保存现有 API 和身份服务的 request 合法域名，再验证网站与小程序双向读写。当前仅有本地打包、自动测试和原网站构建验证，微信编译、真机、账号双向同步及后台提审条件未验收。详见 `wechat/README.md`。
